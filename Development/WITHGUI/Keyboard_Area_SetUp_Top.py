...
class Keyboard_Area_SetUp_Top:
    def __init__(self,root,vid_path,crop_reg,cropped_dim):
        self.root=root
        self.root.title("Set Keyboard Top Bound")
        self.vid = self.VideoCapture(vid_path,crop_reg,cropped_dim)
    ...
    def end_process(self):
        global top_key_bound
        top_key_bound = (self.trackbar_Xa.get(),self.trackbar_Ya.get(),self.trackbar_Xb.get(),self.trackbar_Yb.get())
        self.root.destroy()



    class VideoCapture:
        def __init__(self,vid_path,crop_reg,cropped_dim):
        ...
        def get_frame(self,X_a,Y_a,X_b,Y_b):
            if self.cap.isOpened():
                ...
                if ret:
                    cv.line(cropped_frame,(X_a,Y_a),(X_b,Y_b),(0,255,0), thickness=2) # Top variable line
...
def main(vid_path,crop_reg,cropped_dim):
    root = Tk()
    Keyboard_Area_SetUp_Top(root,vid_path,crop_reg,cropped_dim)
    return top_key_bound
