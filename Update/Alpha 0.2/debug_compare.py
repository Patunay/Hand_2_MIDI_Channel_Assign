...
def Compare(synqued_midi_data,vid_path,crop_reg,crop_dim,trans_matrix,midi_event_with_contours):     # Case by case comparison
    ...
    def vid_mid_comp(cap,onset,midi_event):
        ...
        def check_last_resource(landmarks): # Needs fixing, see debug_notes.txt
            left_distances = []
            right_distances = []
            compare = []
            for hand_id, ind_hand in enumerate(landmarks):  # For each hand
            ...
            compare.append(left_distances)  # something terribly wrong with index-hand assignation
        ...
        print(guess)
        cv.imshow("Compare Debug",trans_frame)
